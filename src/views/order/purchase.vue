<template>
  <div class="app-container">
    <el-card>
      <el-form inline :model="searchKey">
        ...
        <el-form-item>
          ...
          <el-button type="primary" @click="addorupdate('add','')">添加进货订单</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <el-table border :data="tableData" :height="tableHeight">
      ...
      <el-table-column prop="price" label="进货单价" width="180" />
      <el-table-column label="总价" width="180">
        <template slot-scope="scope">
          <span>{{ scope.row.price*scope.row.inNumber }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="content" label="商品描述" width="180" />
      <el-table-column prop="norms" label="商品规格" width="180" />
      <el-table-column prop="intime" label="进货时间" width="180">
        <template slot-scope="scope">
          <span>{{ scope.row.intime| moment() }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="inNumber" label="进货数量" width="180" />
    ...
    />
    <purchase-info :show="dialogShow" :item-data="itemData" @close="closeDialog" />
  </div>
</template>
...
import {
  ...
  deleteOrderPurchasenById
} from '@/api/orderPurchase'
import purchaseInfo from './components/purchaseInfo.vue'
export default {
  components: { purchaseInfo },
  data() {
    return {
      searchKey: {},
      tableData: [],
  ...
  methods: {
    getTableData() {
      ...
      getOrderPurchaseList(params).then((res) => {
        if (res.code === 200) {
          var data = res.data
    ...
    deleteItem(data) {
      this.$confirm('确认永久删除该数据？', '警告', {
        ...
        .then(() => {
          ...
          deleteOrderPurchasenById(params).then((res) => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
                message: '操作成功'
